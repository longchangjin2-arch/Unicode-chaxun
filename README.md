...
- **快捷入口**：一键跳转到搜索、全部字符、随机字符、区块浏览等所有功能
...
### 浏览方式
- **全部字符**：按 Unicode 码点顺序浏览全部有效字符（懒加载按需分页，共约 111 万个）
  - 侧边栏入口：🔠 全部字符
  - 支持跳转到指定页码
  - 支持按 Unicode 编码（如 `4E2D` / `U+4E2D`）跳转定位，自动高亮对应字符
...
- **跳 Unicode**（全部字符视图）：输入十六进制编码（如 `4E2D` / `U+4E2D`），自动定位到对应页并高亮字符
...
- **字符导航**：详情弹窗底部支持前/后切换，键盘 ← → 也可切换（全部字符视图支持跨页导航）
...
### 个人数据管理
- **收藏状态显示**：每个字符右上角的 ★/☆ 图标显示是否已收藏（纯状态标识，不可点击）
- **本页批量收藏**：在所有网格视图（搜索结果、全部字符、区块浏览、历史、随机）顶部：
  - `⭐ 收藏本页` — 一键收藏当前页全部字符（约 96 个）
  - `☆ 取消本页收藏` — 本页全部已收藏时切换为红色取消按钮
  - 进度显示：`⭐ 42 / 96 已收藏`
  - 部分已收藏时还有「收藏未收藏的 N 个」快捷按钮
- **收藏分类**：创建自定义分类，像视频收藏夹一样管理收藏的字符
- **自定义字义**：管理模式下编辑字符含义
- **自定义字典**：管理模式下编辑拼音、笔画、部首
- **操作日志**：记录最近的操作行为，便于追溯（批量收藏会记录）
